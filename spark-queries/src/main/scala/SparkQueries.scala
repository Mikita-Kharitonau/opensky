...
object SparkQueries {
  def main(args: Array[String]) {
    ...
    val rowDf = spark.read.format("csv").option("inferSchema", "true").load("file:///home/nikitakharitonov/Projects/opensky/opensky-data-2019-08-29/*/*")
    ...
    println("spark.sql(\"select origin_country, avg(latitude) from opensky_data group by origin_country\").show")
    ...
    println("spark.sql(\"select origin_country, min(longitude) from opensky_data group by origin_country\").show")
    ...
    println("df.filter(allAfterPredicate(4) && (col(\"velocity\") !== \"None\")).withColumn(\"velocity\", col(\"velocity\") cast \"Double\").agg(max(\"velocity\")).show")
    df.filter(allAfterPredicate(4) && (col("velocity") =!= "None")).withColumn("velocity", col("velocity") cast "Double").agg(max("velocity")).show

    println("spark.sql(s\"select max(cast(velocity as double)) from opensky_data where time > ${allAfter(4)} and velocity != 'None'\").show")
    ...
    println("spark.sql(s\"select icao24, avg(velocity) as velocity from opensky_data where time > ${allAfter(4)} group by icao24\").sort(col(\"velocity\").desc).limit(10).show")
    ...
    println("spark.sql(s\"select icao24, avg(velocity) as velocity from opensky_data where time > ${allAfter(4)} group by icao24 sort by velocity desc limit 10\").show")
    ...
    println("spark.sql(s\"select distinct icao24 from opensky_data where time > ${allAfter(4)} and origin_country = 'Germany' and on_ground = 'True'\").show")
    ...
    println("spark.sql(s\"select origin_country, count(*) as cnt from (select icao24, origin_country from opensky_data where time > ${allAfter(240)} and on_ground = 'False' group by icao24, origin_country) group by origin_country sort by cnt desc limit 10\").show -- It is seems not working, don't understand why..")
    spark.sql(s"select origin_country, count(*) as cnt from (select icao24, origin_country from opensky_data where time > ${allAfter(24)} and on_ground = 'False' group by icao24, origin_country) group by origin_country sort by cnt desc limit 10").show
    ...
    spark.sql(s"select origin_country, count(*) as cnt from (select icao24, origin_country from opensky_data where time > ${allAfter(24)} and on_ground = 'False' group by icao24, origin_country) group by origin_country").sort(col("cnt").desc).limit(10).show
    ...
    println("spark.sql(s\"select origin_country, round(avg(geo_altitude), 3) as avg_geo_altitude from opensky_data group by origin_country\").show")
  ...
  @tailrec
  final def maxInterval(times: java.util.List[Integer], index: Int, current: Long): Long = {
    if(index == times.size - 1) return current
    val actual = times.get(index + 1) - times.get(index)
